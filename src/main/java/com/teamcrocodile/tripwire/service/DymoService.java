...
public class DymoService {
    ...
    private int scoreFromApiResponse(DymoResponse response) {
        ...
        int score = 0;
        score += scoreRule(response.getEmail() != null && response.getEmail().isFraud(), 25, "Dymo flagged email.fraud", response);
        score += scoreRule(response.getEmail() != null && response.getEmail().isProxiedEmail(), 25, "Dymo flagged email.proxiedEmail", response);
        ...
        score += scoreRule(response.getDomain() != null && response.getDomain().isFraud(), 35, "Dymo flagged domain.fraud", response);

        score += scoreRule(response.getIp() != null && response.getIp().isFraud(), 35, "Dymo flagged ip.fraud", response);
        score += scoreRule(response.getIp() != null && response.getIp().isVpn(), 13, "Dymo flagged ip.vpn", response);
        score += scoreRule(response.getIp() != null && response.getIp().isProxy(), 13, "Dymo flagged ip.proxy", response);

        score += scoreRule(response.getPhone() != null && response.getPhone().isFraud(), 35, "Dymo flagged phone.fraud", response);
        score += scoreRule(response.getIban() != null && response.getIban().isFraud(), 35, "Dymo flagged iban.fraud", response);
        score += scoreRule(isFraud(response.getCreditCard()), 35, "Dymo flagged creditCard.fraud", response);
        score += scoreRule(isFraud(response.getWallet()), 35, "Dymo flagged wallet.fraud", response);
        score += scoreRule(isFraud(response.getUserAgent()), 30, "Dymo flagged userAgent.fraud", response);
